...
\item
  Como essa performance se relaciona com características das
  distribuições dos regimes?
\end{itemize}

\pause

Mudança de regime (RS): comportamento que alterna entre dois ou mais
padrões; DGP alterna entre conjuntos de parâmetros.

\pause

Comportamento observável nos dados, e útil para modelar
não-linearidades, outliers, etc.
\end{frame}
...
\small Lima et al.~(2007) ``Monetary policy regimes in Brazil''
...
\caption{\label{fig-ws_forecast}Produção de energia no noroeste dos EUA}
...
\small Fonte: Fonte: Gneiting et al.~(2012) ``Calibrated Probabilistic
...
Estudar a performance desses modelos é importante, e as características
...
\caption{\label{fig-ws_boxplots}Produção de energia no noroeste dos EUA
- Distribuições}
...
\item
  \emph{Markov Switching}, \emph{Self-Exciting Threshold}, \emph{Smooth
  Transition}.
\item
...
\item
  MS, SET, e ST.
\item
  K-means não supervisionado e Random Forest. Com 4 lags de \(y\),
  média, desvio-padrão, e \(ACF(1)\) móveis.
\item
...
\item
  Condensadas pela distância média:
...
\begin{frame}{Proporções dos regimes}
\phantomsection\label{proporuxe7uxf5es-dos-regimes}
\begin{figure}

\caption{\label{fig-regimes_est}Proporções dos regimes}

\centering{

\includegraphics[width=\linewidth,height=0.75\textheight,keepaspectratio]{../../outputs/diagnostics/regimes_est.pdf}
...
\phantomsection\label{visuxe3o-geral}
...
\begin{frame}{Diferenças entre os regimes}
\phantomsection\label{diferenuxe7as-entre-os-regimes-1}
\begin{figure}

\caption{\label{fig-rs-ms}Separação dos regimes variando \(T\) - MS}

\centering{

\includegraphics[width=\linewidth,height=0.75\textheight,keepaspectratio]{../../outputs/exploratory/metrics_sep_ms.pdf}
...
\begin{frame}{Má-especificação e performance}
\phantomsection\label{muxe1-especificauxe7uxe3o-e-performance}
...
\begin{frame}{Identificação e performance}
\phantomsection\label{identificauxe7uxe3o-e-performance}
\begin{table}[!htbp]
...
\begin{frame}{Identificação e performance}
\phantomsection\label{identificauxe7uxe3o-e-performance-1}
\begin{table}[!htbp]
...
\begin{itemize}
\item
  Objetivo: estudar os processos e modelos de RS, através das
  características das distribuições de seus regimes.
\item
...
\tightlist
\item
  RGPs e RNs interagem significantemente, e as métricas condicionais
...
\item
  ...
  diferentes DGPs, e boas previsões mesmo com \(r\)s imprecisos.
\item
...
\item
  ...
  em \(\rho\), e erra a volatilidade condicional.
\item
  MS lida com regimes assimétricos, sofre com má-especificações do RGP e
  erra métricas condicionais.
\end{itemize}

\pause

Resultados apresentam fatos úteis para entender processos e modelos. A
análise de modelos e métricas tem potencial para recomendações práticas.
\end{frame}
...
entre todos RGPs e RNs; e difícil para os modelos estimarem.
