...
diagnose_errors <- function(errors) {
  ...
  ((g_hist - g_qq) * theme(plot.title = element_text(size = 12))) +
    plot_annotation(title = "Error distribution (sample of 3000)")
}
...
diagnose_series <- function(gdata, only_one = FALSE) {
  if (only_one) gdata <- gdata %>% filter(sim == 1)
  ...
  ggplot(gdata, aes(x = t, y = y)) +
    ...
    labs(title = "Simulated series - values", x = "Time", y = "Value", color = "Regime")
}


diagnose_paths <- function(gdata, t_max = 30) {
  gdata_segment <- gdata %>%
    filter(sim == 1 & t %in% 1:t_max) %>%
    group_by(sgp, rgp) %>%
    mutate(
...
diagnose_regimes <- function(gdata, t_max = n_t) {
  gdata_segment <- gdata %>%
    filter(sim == 1 & t %in% 1:t_max) %>%
    group_by(sgp, rgp) %>%
    mutate(
  ...
  ggplot(gdata_segment, aes(x = t, y = r)) +
    ...
    labs(title = "Simulated series - regimes", x = "Time", y = "Regime")
}
